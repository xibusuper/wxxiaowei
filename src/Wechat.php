 ...
 * 微信小微商户SDK
 */
if (!class_exists("BaseWechat")) {
    include_once "3rd/BaseWechat.php";
}

class Wechat
{
    //开放平台创建绑定张号  绑定  解绑 为了同一个unionid
    const API_URL_OPEN                  = 'https://api.weixin.qq.com'; // 开放平台

    public $debug   = false;
    private $_logcallback;
    private $baseWechat = null;
    ...
    {
        if(!$this->baseWechat){
            $this->baseWechat = new \BaseWechat($wechatConfig);
        }
    }
    ...
    public function applyEnter(array $params)
    {
        ...
        if (!$this->checkParams($params)) {
            $this->return_err(20004);
        }
        // 校验银行卡号前缀是否支持
        if ($this->accountNumberIsSupport($params['account_number'] ? $params['account_number'] : '')) {
            $this->return_err(20003);
        }
        $data = [
            'version'                => '2.0',
            ...
            'store_longitude'        => $params['store_longitude'] ? $params['store_longitude'] : '',
            'store_latitude'         => $params['store_latitude'] ? $params['store_latitude'] : '',
            'store_entrance_pic'     => $params['store_entrance_pic'],
            'indoor_pic'             => $params['indoor_pic'],
            'address_certification'  => $params['address_certification'] ? $params['address_certification'] : '',
            'merchant_shortname'     => $params['merchant_shortname'],
            'service_phone'          => $params['service_phone'],
            'business'               => $params['business'],
            'product_desc'           => $params['product_desc'] ? $params['product_desc'] : '',
            'qualifications'         => $params['qualifications'] ? $params['qualifications'] : '',
            'rate'                   => $params['rate'],
            'business_addition_desc' => $params['business_addition_desc'] ? $params['business_addition_desc'] : '',
            'business_addition_pics' => $params['business_addition_pics'] ? $params['business_addition_pics'] : '',    // ["123","456"] 最多可上传5张照片，请填写已预先上传图片生成好的MediaID
    ...
    public function enquiryOfApplyStatus(array $params)
    {
        ...
        $data         = [
            ...
            'sign_type'     => 'HMAC-SHA256',
            'sign'          => '',
            'applyment_id'  => $params['applyment_id'] ? $params['applyment_id'] : '',
            'business_code' => $params['business_code'] ? $params['business_code'] : '',
        ];
        $data['sign'] = $this->makeSign($data, $data['sign_type']);
    ...
    public function payTheDirectoryConfig(array $params)
    {
        ...
        if (!isset($params['jsapi_path'])) {
            $this->return_err(20004, '授权目录必填');
        }
        if (!isset($params['appid'])) {
            $this->return_err(20004, '服务商的公众账号 ID 必填');
        }
        $data         = [
            'appid'      => $params['appid'],
            'mch_id'     => $this->mch_id,
    ...
    public function bindAppIdConfig(array $params)
    {
        ...
        if (!isset($params['sub_appid'])) {
            $this->return_err(20004, '关联 APPID 必填');
        }
        if (!isset($params['appid'])) {
            $this->return_err(20004, '服务商的公众账号 ID 必填');
        }
    ...
    private function httpPost($url, $param = "", $post_file = false)
    {
        ...
        if (intval($aStatus["http_code"]) == 200) {
            $this->log("wxopen httpPost: {$strPOST} recv:" . $sContent);
            return $sContent;
        } else {
            $this->log("wxopen httpPost: {$strPOST} recv error {$url}, param:{$param} aStatus:" . print_r($aStatus, true));
            return false;
        }
    }
    ...
    private function return_err($code=-1,$info=''){
